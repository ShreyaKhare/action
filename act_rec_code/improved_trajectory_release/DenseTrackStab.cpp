...
int main(int argc, char** argv)
{
	...
	ofile.write((char*)&feat_num ,sizeof(float));
	ofile.write((char*)&dim ,sizeof(float));
			...
			for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
				...
				if(iTrack->index >= trackInfo.length) {
					...
					if(IsValid(trajectory, mean_x, mean_y, var_x, var_y, length) && IsCameraMotion(displacement)) {
						...
						float tmp1=std::min<float>(std::max<float>(mean_x/float(seqInfo.width), 0), 0.999);
						//printf("%f\t", std::min<float>(std::max<float>(mean_y/float(seqInfo.height), 0), 0.999));
						float tmp2=std::min<float>(std::max<float>(mean_y/float(seqInfo.height), 0), 0.999);
						//printf("%f\t", std::min<float>(std::max<float>((frame_num - trackInfo.length/2.0 - start_frame)/float(seqInfo.length), 0), 0.999));
						float tmp3=std::min<float>(std::max<float>((frame_num - trackInfo.length/2.0 - start_frame)/float(seqInfo.length), 0), 0.999);

						float frame_num_f=(float)frame_num;
						ofile.write((char*)&frame_num_f ,sizeof(float));
						ofile.write((char*)&mean_x ,sizeof(float));
						ofile.write((char*)&mean_y ,sizeof(float));
						ofile.write((char*)&var_x ,sizeof(float));
						ofile.write((char*)&var_y ,sizeof(float));
						ofile.write((char*)&length ,sizeof(float));
						ofile.write((char*)&fscales[iScale] ,sizeof(float));
						ofile.write((char*)&tmp1 ,sizeof(float));
						ofile.write((char*)&tmp2 ,sizeof(float));
						ofile.write((char*)&tmp3 ,sizeof(float));
						// output the trajectory
						for (int i = 0; i < trackInfo.length; ++i){
							float disp_x_f=(float)displacement[i].x;	
							float disp_y_f=(float)displacement[i].y;	
							ofile.write((char*)&disp_x_f ,sizeof(float));
							ofile.write((char*)&disp_y_f ,sizeof(float));
					...
					//	PrintDesc(iTrack->mbhY, mbhInfo, trackInfo);
						...
						PrintDesc_bin(iTrack->hog, hogInfo, trackInfo,ofile);
						PrintDesc_bin(iTrack->hof, hofInfo, trackInfo,ofile);
						PrintDesc_bin(iTrack->mbhX, mbhInfo, trackInfo,ofile);
						PrintDesc_bin(iTrack->mbhY, mbhInfo, trackInfo,ofile);
						feat_num++;

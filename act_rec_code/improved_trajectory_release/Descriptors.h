...
void PrintDesc_bin(std::vector<float>& desc, DescInfo& descInfo, TrackInfo& trackInfo,std::ofstream& ofile)
{
	int tStride = cvFloor(trackInfo.length/descInfo.ntCells);
	float norm = 1./float(tStride);
	int dim = descInfo.dim;
	...
	for(int i = 0; i < descInfo.ntCells; i++) {
		...
		for(int j = 0; j < dim; j++){
//			printf("%.7f\t", vec[j]*norm);
			float out=vec[j]*norm;
			ofile.write((char*)&out ,sizeof(float));
		}
	}
}
